...
class MetadataPanel:
    ...
    def build(self, parent) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(meta, text="Name:").grid(row=1, column=0, sticky="w", pady=2)
        self.meta_name = ctk.CTkEntry(meta)
        self.meta_name.insert(0, doc.name)
        self.meta_name.grid(row=1, column=1, sticky="ew", pady=2)

        ctk.CTkLabel(meta, text="Author:").grid(row=2, column=0, sticky="w", pady=2)
        ...
        ctk.CTkLabel(meta, text="Animation FPS:").grid(row=3, column=0, sticky="w", pady=2)
        ...
        ctk.CTkLabel(meta, text="Loop:").grid(row=4, column=0, sticky="w", pady=2)
        self.meta_loop = ctk.BooleanVar(value=doc.loop)
        ctk.CTkCheckBox(meta, text="", variable=self.meta_loop).grid(
            row=4, column=1, sticky="w", pady=2
        )

        ctk.CTkLabel(meta, text="Tags:").grid(row=5, column=0, sticky="w", pady=2)
        ...
        self.prop_collision = ctk.BooleanVar(value=props.get("collision", False))
        self.prop_static = ctk.BooleanVar(value=props.get("static", False))
        self.prop_background = ctk.BooleanVar(value=props.get("background", False))
        self.prop_player = ctk.BooleanVar(value=props.get("player", False))

        ctk.CTkCheckBox(flags_box, text="Collision", variable=self.prop_collision).grid(sticky="w")
        ctk.CTkCheckBox(flags_box, text="Static Asset", variable=self.prop_static).grid(sticky="w")
        ctk.CTkCheckBox(flags_box, text="Background", variable=self.prop_background).grid(sticky="w")
        ctk.CTkCheckBox(flags_box, text="Player Character", variable=self.prop_player).grid(sticky="w")

        for entry in (self.meta_name, self.meta_author, self.meta_fps, self.meta_tags):
    ...
    def show_saved_status(self) -> None:
        ...
        self.editor.after(1500, _remove_label)
    ...
    def refresh_from_doc(self) -> None:
        ...
        with self.suspend_autoapply():
            ...
            if self.prop_background:
                self.prop_background.set(doc.properties.get('background', False))
            if self.prop_player:
